...
| `dedupe` | script | Drops anything already in SQLite, by URL and by normalized title over the last 100 articles. Caps the run at 25 new articles, shared round-robin across the sources that returned any, so a busy feed cannot crowd out a quiet one. |
