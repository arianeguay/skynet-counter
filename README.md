...
|---|---|---|
| `fetch` (map over `input.feeds`) | script ×5 | One `fetch-feed` sub-pipeline run per feed — Ars Technica Security, Ars Technica AI, The Hacker News, Krebs on Security, HN. The list lives in `.studio/inputs/default.input.yaml`, so adding a feed is two lines there. A feed marked `hydrate: true` there ships no article text in its summaries — the stage reads each linked page and scores against that instead. A feed that errors emits an empty batch with the reason, so one publisher's 502 does not take the sweep down. |
