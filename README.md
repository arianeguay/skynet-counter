...
|---|---|---|
| `fetch` (parallel group) | script ×5 | One fetcher per feed — Ars Technica Security, Ars Technica AI, The Hacker News, Krebs on Security, HN. A feed that errors emits an empty batch with the reason, so one publisher's 502 does not take the sweep down. (A parallel group reports `failed` on any failed stage — `on_failure: collect-all` only keeps the siblings running.) |
