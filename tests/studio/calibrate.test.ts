...
function dbWith(rows: { score: number | null; published_at: string }[]) {
  const path = join(mkdtempSync(join(tmpdir(), 'skynet-cal-')), 'x.db');
  const db = new Database(path);
  db.run('CREATE TABLE articles (url TEXT PRIMARY KEY, published_at TEXT NOT NULL, score INTEGER)');
  rows.forEach((r, i) => db.run('INSERT INTO articles VALUES (?, ?, ?)', [`u${i}`, r.published_at, r.score]));
  db.close();
  return path;
}
...
test('the grid reproduces the counter the formula would publish', async () => {
  ...
  expect(code).toBe(0);
  // 20 at age 0 plus 10 at one half-life = 20 + 5 = 25 signal, so 12 + 25/8.
  const expected = counterFrom(decayedSignal(rows.map((r) => ({ ...r, score: r.score })), Date.now()));
  expect(expected).toBeCloseTo(15.1, 1);
